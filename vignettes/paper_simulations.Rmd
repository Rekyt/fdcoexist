...
extract_trait_contrib_from_scenar = function(scenario) {
    R_scenar = as.numeric(sub("R(\\d+).*", "\\1", scenario, perl = TRUE))
    A_scenar = as.numeric(sub(".*A(\\d+).*", "\\1", scenario, perl = TRUE))
    H_scenar = as.numeric(sub(".*H(\\d+)", "\\1", scenario, perl = TRUE))
...
extract_simul_param = function(single_simul, time = 150) {
    
    scenario = single_simul$scenario
    
    contrib = extract_trait_contrib_from_scenar(scenario)
    
    population_df = single_simul$compo[[1]][,,time] %>%
        as.data.frame() %>%
...
tidy_simulation = function(scenario, given_simul, given_traits,
    ...
    extract_simul_param(given_simul[[scenario]], given_traits,
