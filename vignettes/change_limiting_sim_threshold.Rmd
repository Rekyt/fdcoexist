...
# Create traits
trait_df = list(
    list(uncor = matrix(c(seq(1, 19, length.out = 4) %>%
                            rep(each = 3) + c(0, 1, 2),
                        rnorm(n_species)), ncol = 2,
         dimnames = list(species = paste0("species", seq(12)),
                         trait   = paste0("trait", 1:2))))
)

# Trait Scenario
trait_weights = list(R0A0H0 = create_trait_weights(0, 0, 0, n_traits = 2)[1,])
