...
compute_site_cwm_cwv = function(simulation_df) {
    simulation_df %>%
        group_by(seed, trait_cor, k, h_fun, A, B_over_A, H, R_scenar, A_scenar,
                 H_scenar, patch) %>%
        summarise_at(vars(matches("trait\\d")),
                     funs(cwm = weighted.mean(., N),
                          cwv = Hmisc::wtd.var(., N)))
}

extract_performances_from_simul = 
         function(simul, trait_df, compute_realized_growth_rate = TRUE) {
...
## Coexistence plot depending on A, B/A, k and H
...
var_ba_df = lapply(var_a_var_k_var_ba, function(x) {
    x[[1]] %>%
            ...
            mutate(k             = x[[1]]$k,
                   ...
                   seed_number   = 1) %>%
            select(k, A, B_over_A, H, seed_number, compet_status,
        ...
        group_by(k, A, B_over_A, H, seed_number, compet_status,
...
var_ba_r2 = var_ba_df %>%
    ...
    nest(-k, -A,-B_over_A, -H, -seed_number, -scenario) %>%
    mutate(data = data %>%
               map(~.x %>%
...
plot_k_H_r2 = plot_average_r_squared(var_ba_r2, "k", "H")

plot_A_H_r2 = plot_average_r_squared(var_ba_r2, "A", "H") +
    scale_x_log10()

plot_BA_H_r2 = plot_average_r_squared(var_ba_r2, "B_over_A", "H") +
    scale_x_log10()
...
var_ba_df %>%
    ...
    geom_line() +
    facet_grid(vars(A), vars(k, H), labeller = label_context) +
    labs(x = "Environment",
         y = "Trait 2 CWM")
```
...
var_scenars = list(seed     = seq(1),
    ...
    set_names(nm = list(c("gaussian", "fixed"), c("sum", "prod")) %>%
                  cross() %>%
...
var_scenars_df = map_dfr(
    var_scenars,
    ~.x %>%
        set_names(nm = names(scenar_list)) %>%
        tidy_simulation_list(used_traits, scenar_list,
                             scenar_names = names(scenar_list)) %>%
        mutate(R_scenar = "R(\\d+)A\\d+H\\d+" %>%
                   gsub("\\1", scenario, perl = TRUE) %>%
                   as.numeric(),
               A_scenar = "R\\d+A(\\d+)H\\d+" %>%
                   gsub("\\1", scenario, perl = TRUE) %>%
                   as.numeric(),
               H_scenar = "R\\d+A\\d+H(\\d+)" %>%
                   gsub("\\1", scenario, perl = TRUE) %>%
                   as.numeric()) %>%
        select(scenario, R_scenar, A_scenar, H_scenar, patch_optim,
               everything()) %>%
        select(-growth_trait, -compet_dist, -optim_dist, -growth_trait_cwm,
               -patch) %>%
        rename(patch = patch_optim),
    .id = "k_type")

var_scenars_cwm = var_scenars_df %>%
    group_by(k_type, scenario, R_scenar, A_scenar, H_scenar, patch) %>%
    summarise(trait1_mwc = weighted.mean(trait1, N),
              trait2_mwc = weighted.mean(trait2, N),
              trait3_mwc = weighted.mean(trait3, N),
              trait4_mwc = weighted.mean(trait4, N),
              trait1_cwm = sum(trait1_cwm),
              trait2_cwm = sum(trait2_cwm),
              trait3_cwm = sum(trait3_cwm),
              trait4_cwm = sum(trait4_cwm)) %>%
    separate(k_type, c("k_type", "h_fun"), sep = "_")
```
...
var_scenars_cwm %>%
    ...
    geom_abline(slope = 1, intercept = 0, linetype = 2) +
    geom_line(size = 1) +
    ggpmisc::stat_poly_eq(aes(label = ..eq.label..), formula = y ~ x,
                          parse = TRUE, size = 3, label.y.npc = "bottom",
                          label.x.npc = "right") +
    facet_grid(vars(R_scenar), vars(h_fun, A_scenar),
               labeller = labeller(R_scenar = function(x) paste0("Trait 2 Contrib.\nto Growth : ", x, "%"),
    ...
    labs(x = "Environmental Variable",
         y = "CWM of Trait 2",
         color = "Trait 2 Contrib.\nto Hierarch. Compet.",
         subtitle = expression("Trait 2 CWM <-> Environment (t"["opt"] *
                                   ") relationship"),
         caption = paste0("No trait correlation; fixed k = ", fixed_k,
                          "; A = ", fixed_A, "; B/A = ", fixed_B_over_A,
                          "; additive H; H = ", fixed_H)) +
