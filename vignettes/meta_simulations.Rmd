...
multi_traits_fixed = cross(
    list(run_n = seq(1),
         A = fixed_A, k = fixed_k, B_over_A = fixed_B_over_A,
         H_over_k = fixed_H_over_k)) %>%
    future_map(function(x) {
        devtools::load_all()
...
multi_traits_triangle = map2(multi_traits_fixed, trait_list,
                             function(simul, trait_df) {
    
    given_compo = simul[[1]]$compo[[1]]
...
fig_double_facet = function(given_df, x_var, y_var, col_var, given_labs,
                            given_label_pos = "left") {
    given_df %>%
        filter(patch %in% seq(1, 25, by = 2)) %>%
    ggplot(aes_string(x_var, y_var, color = col_var)) +
        ...
        facet_wrap(~patch + trait_cor, ncol = 12,
