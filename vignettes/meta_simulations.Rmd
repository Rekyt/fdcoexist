...
extract_simul_param_and_abund = function(single_simul, time = 150) {
    ...
    single_simul$compo[[1]][,,time] %>%
        ...
        mutate(rel_abund = N/sum(N),
               ...
               di_thresh = single_simul$di_thresh) %>%
        ungroup() %>%
        select(seed, trait_cor, k, h_fun, A, B, H, R_scenar, A_scenar,
               H_scenar, everything())
}
...
compute_site_cwm_cwv_sr = function(simulation_df) {
    ...
    group_site = simulation_df %>%
        group_by(seed, trait_cor, k, h_fun, A, B, H, di_thresh, R_scenar,
                 A_scenar, H_scenar, patch)
    
    rich_df = group_site %>%
        summarise(species_rich = sum(N > 0, na.rm = TRUE),
                  num_indiv    = sum(N, na.rm = TRUE))
    
    group_site %>%
        summarise_at(vars(tidyselect::matches("trait\\d")),
                     list(cwm = ~weighted.mean(., N, na.rm = TRUE),
                          cwv = ~Hmisc::wtd.var(., N, na.rm = TRUE),
                          cws = ~wtd_skewness(., N, na.rm = TRUE),
                          cwk = ~wtd_kurtosis(., N, na.rm = TRUE),
                          trait_range = ~diff(range(ifelse(N > 0, ., NA),
                                                   na.rm = TRUE)))) %>%
        inner_join(rich_df) %>%
        ungroup()
}
...
extract_performances_from_simul = function(simul, trait_list,
    ...
    trait_df = trait_list[[simul$seed]][[cor_scenar]]
...
n_gen     <- 75   # Length of model run (generations)
...
triangle_labs = c(N150                   = "Abundance at time = 150",
                  ...
                  null_max_gr_per_capita = "Max. Growth Rate in Monoculture")

var_labs = c(k        = "k (maximum growth rate)",
             A        = "A (interspecific competition)",
             B        = "B (intraspecific competition)",
...
list_A = 10^-(seq(1, 8, length.out = 6))
...
var_a_var_k_var_ba = list(
    ...
    future_map(function(x) {
        ...
        meta_simul_A_B_k_and_H(x$run_n, x$k, x$A, x$B,
                               given_H = x$H)
    }, .progress = TRUE)
tictoc::toc()
```
...
plot_species_richness = function(var_df, x_var, y_var, given_labs = var_labs) {
    ...
    var_df %>%
        ...
        facet_wrap(~patch) +
        scale_fill_viridis_c(limits = c(0, 53)) +
...
plot_A_B = var_ba_cwm_sr_df %>%
    filter(patch %in% c(1, 13)) %>%
    plot_species_richness("A", "B") +
    scale_x_log10() +
    scale_y_log10()

plot_k_B = var_ba_cwm_sr_df %>%
    filter(patch %in% c(1, 13)) %>%
    plot_species_richness("k", "B") +
    scale_y_log10()
...
plot_B_H = var_ba_cwm_sr_df %>%
    filter(patch %in% c(1, 13)) %>%
    plot_species_richness("B", "H") +
    scale_x_log10()
...
var_ba_slope = var_ba_cwm_sr_df %>%
    ...
    nest(-seed, -trait_cor, -k, -A,-B, -H, -R_scenar, -A_scenar,
         -H_scenar) %>%
    mutate(data = data %>%
               map(~.x %>%
...
plot_average_slope = function(var_df, x_var, y_var, given_labs = var_labs) {
    ...
    var_df %>%
        ...
        labs(x = var_labs[[x_var]],
             y = var_labs[[y_var]],
             fill = "CWM <-> Env. Slope",
             subtitle = paste0("Avg. slope over values of ",
                               paste(not_used_vars, collapse = " and "))) +
        theme(legend.position = "top",
              legend.text = element_text(angle = 45),
              aspect.ratio = 1)
...
plot_A_B_slope = plot_average_slope(var_ba_slope, "A", "B") +
    scale_x_log10() +
    scale_y_log10()

plot_k_B_slope = plot_average_slope(var_ba_slope, "k", "B") +
    scale_y_log10()
...
plot_B_H_slope = plot_average_slope(var_ba_slope, "B", "H") +
    scale_x_log10()
...
```{r cwm_env_param}
var_ba_cwm_sr_df %>%
    filter(k %in% c(1.125, 1.5), H %in% c(0, 1, 2), A <= 1e-3, A >= 1e-5) %>%
    ggplot(aes(patch, trait2_cwm, color = as.factor(B))) +
...
fixed_A = c(1.5e-6, 1e-5)
fixed_k = c(1.3, 1.1)
fixed_B = 1.5e-4
fixed_H = c(0.4, 1.0)
fixed_di_thresh = c(2, 8, 25)
n_seed  = 10
...
var_scenars_df = var_scenars %>%
    map2_dfr(rep(used_trait_list, n_simul/n_seed),
             ~tidy_simulation_list(.x, .y, n_gen))

var_scenars_cwm = var_scenars_df %>%
    compute_site_cwm_cwv_sr()

var_scenars_perf = future_map_dfr(
    unlist(var_scenars, recursive = FALSE),
    ~extract_performances_from_simul(.x, used_trait_list, TRUE),
    .progress = TRUE)
```
...
plot_var_scenars_cwm = function(var_scenars_cwm) {
    ...
    var_scenars_cwm %>%
        ...
        ggpmisc::stat_poly_eq(aes(label = ..eq.label.., group = NULL),
                              ...
                              coef.digits = 2) +
        facet_grid(vars(di_thresh, R_scenar), vars(k, H, A, A_scenar),
                   labeller = labeller(
...
```{r scenar_cwv}
plot_var_scenars_cwv = function(var_scenars_cwm) {
    
    param_caption = extract_caption(var_scenars_cwm)
    
    var_scenars_cwm %>%
        filter(patch >= 5, patch <= 20) %>%
        ggplot(aes(patch, trait2_cwv, color = as.factor(H_scenar),
                   # specify interaction otherwise it messes with groups
                   group = interaction(as.factor(H_scenar), seed))) +
        geom_line(size = 1, alpha = 1/20) +
        geom_smooth(size = 1, se = FALSE, aes(group = NULL)) +
        ggpmisc::stat_poly_eq(aes(label = ..eq.label.., group = NULL),
                              formula = y ~ x, parse = TRUE, size = 3,
                              label.y.npc = "top", label.x.npc = "right",
                              coef.digits = 2) +
        facet_grid(vars(di_thresh, R_scenar), vars(A, A_scenar),
                   labeller = labeller(
                       R_scenar = function(x) paste0("Trait 2 Contrib.\n",
                                                     "to Growth : ", x, "%"),
                       A_scenar = function(x) paste0("Trait 2 Contrib.\nto ",
        ...
        labs(x = "Environmental Variable",
             y = "CWV of Trait 2",
             color = "Trait 2 contrib. to Hierarch. Compet.",
             subtitle = expression("Trait 2 CWV <-> Environment (t"["opt"] *
                                       ") relationship"),
             caption = param_caption) +
        theme(aspect.ratio = 1,
              legend.position = "top")
...
plot_var_scenars_cwv(var_scenars_cwm)
```

```{r scenar_richness}
plot_var_scenars_richness = function(var_scenars_cwm) {
    
    param_caption = extract_caption(var_scenars_cwm)
    
    var_scenars_cwm %>%
        filter(patch >= 5, patch <= 20) %>%
        ggplot(aes(patch, species_rich, color = as.factor(H_scenar),
                   # specify interaction otherwise it messes with groups
                   group = interaction(as.factor(H_scenar), seed))) +
        geom_line(size = 1, alpha = 1/20) +
        geom_smooth(size = 1, se = FALSE, aes(group = NULL),
        ...
        ggpmisc::stat_poly_eq(aes(label = ..eq.label.., group = NULL),
                              formula = y ~ x + I(x^2), parse = TRUE, size = 3,
        ...
        labs(x = "Environmental Variable",
             y = "Species Richness",
             color = "Trait 2 contrib. to Hierarch. Compet.",
             subtitle = expression("Species Richness <-> Environment (t"["opt"] *
                                       ") relationship"),
             caption = param_caption) +
        theme(aspect.ratio = 1,
              legend.position = "top")
...
plot_var_scenars_richness(var_scenars_cwm)
```
...
# Generate trait list
uncor_list = lapply(seq(n_seed), function(given_seed) {
    set.seed(given_seed)
    
    given_traits = generate_cor_traits(n_patches, n_species, n_traits - 1,
                                       cor_coef = 0)
    
    return(list(uncor = given_traits))
})
...
# Actual simulation
tictoc::tic()
multi_traits_fixed = list(seed = seq(n_seed),
                          k    = fixed_k,
                          A    = c(fixed_A, 0),
                          B    = fixed_B,
                          H    = c(fixed_H, 0)) %>%
...
multi_traits_triangle = future_map2_dfr(
    multi_traits_fixed, flatten(c(uncor_list, uncor_list, uncor_list,
                                  uncor_list)),
    ~extract_performances_from_simul(.x[[1]], .y, TRUE), .progress = TRUE)
```
...
param_caption = extract_caption(multi_traits_triangle)
...
### Compute Null Maximum Growth Rate per Capita

```{r compute_null_growth_rate}
null_gr = multi_traits_triangle %>%
    filter(A == 0, H == 0) %>%
    select(seed, patch, species, max_growth_rate,
           max_growth_rate_per_capita) %>%
    rename(null_max_gr = max_growth_rate,
           null_max_gr_per_capita = max_growth_rate_per_capita)

multi_traits_triangle = multi_traits_triangle %>%
    inner_join(null_gr, by = c("seed", "patch", "species"))

multi_traits_triangle %>%
    filter(A != 0, H != 0) %>%
    fig_double_facet("N150", "null_max_gr_per_capita", "distance_to_optimum",
                     triangle_labs, "right")

given_caption = multi_traits_triangle %>%
    filter(A != 0, H != 0) %>%
    extract_caption()

plot_growth_rate_perf = multi_traits_triangle %>%
    gather("null_growth_rate_type", "null_growth_rate_value",
           null_max_gr) %>%
    filter(A != 0 & H != 0,
           !is.na(null_growth_rate_value), trait_cor == "uncor") %>%
    mutate(null_growth_rate_type = as_factor(null_growth_rate_type)) %>%
    gather("perf_type", "perf_value", N150, distance_to_optimum) %>%
    ggplot(aes(perf_value, null_growth_rate_value, color = null_growth_rate_type)) +
...
plot_growth_rate_perf_percapita = multi_traits_triangle %>%
    gather("null_growth_rate_type", "null_growth_rate_value",
           "null_max_gr_per_capita") %>%
    filter(A != 0 & H != 0,# null_growth_rate_value < 4,
           trait_cor == "uncor", !is.na(null_growth_rate_value),
           !is.infinite(null_growth_rate_value)) %>%
    mutate(null_growth_rate_type = as_factor(null_growth_rate_type)) %>%
    gather("perf_type", "perf_value", N150, distance_to_optimum) %>%
    ggplot(aes(perf_value, null_growth_rate_value, color = null_growth_rate_type)) +
    geom_point(alpha = 0.99, shape = ".") +
    geom_smooth(method = "lm", se = FALSE) +
    ggpmisc::stat_poly_eq(formula = y ~ x, parse = TRUE, label.x.npc = "left",
                          aes(label = ..rr.label..), size = 3) +
    facet_grid(vars(k), vars(perf_type), scales = "free_x",
               labeller = labeller(
                   ...
                   k         = c(
                       "fixed = 1.15" = "k = 1.15",
                       gaussian = "k ~> N(1.5, 0.3)"))) +
    scale_color_manual(values = RColorBrewer::brewer.pal(3, "Dark2")[2],
                       labels = c(null_max_gr = "Null growth rate")) +
    labs(x = "Performance Value", y = "Growth Rate", color = NULL,
         subtitle = "Growth Rates Estimates vs. other Performance Indices",
         caption = given_caption) +
    theme(legend.position = "top",
          panel.grid = element_blank(),
          aspect.ratio = 1)
plot_growth_rate_perf_percapita
