...
plot_predictions_along_environment = function(perf_along_env_df) {
    perf_along_env_df %>%
        ...
        ggpmisc::stat_poly_eq(formula = y ~ x, parse = TRUE, size = 2.4,
                              aes(label = paste(..rr.label.., ..eq.label..,
                                                sep = "~~"))) +
...
# Save files -------------------------------------------------------------------
ggsave("perf_environment_trait2_only.png", fig_first_scenar,
...
# Save files -------------------------------------------------------------------
ggsave("perf_environment_both_traits_only_growth.png", fig_second_scenar,
...
# Save files -------------------------------------------------------------------
ggsave("perf_environment_trait1_growth_trait2_competition.png", fig_third_scenar,
       width = 17, height = 25.5, units = "cm")
...
# Save files -------------------------------------------------------------------
ggsave("perf_environment_both_traits_all.png", fig_fourth_scenar,
       width = 17, height = 25.5, units = "cm")
...
# 
# indiv_mod = big_mod %>%
#     nest(seed, patch, trait_value) %>%
#     mutate(lin_mod = map(data, ~lmer(patch ~ trait_value + (1|seed), data = .)),
#            mod_coef = map(lin_mod, broom::tidy))
...
single_mod = big_mod %>%
    ...
    mutate(data = map(data, ~mutate_at(.x,
                                       c("k", "A", "B", "H"),
                                       scale)),
           lin_mod = map(data, ~lmerTest::lmer(trait_value - patch ~  (k + A +
                                                   B + H)^2 + (1|seed),
                                               data = .)))
```
