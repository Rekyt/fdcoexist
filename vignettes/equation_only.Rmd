...
\begin{equation}
\delta_{ij} = \sqrt{\sum_{c = 1}^{T_c} (t_{i, c} - t_{j, c})^2}, 
\end{equation}

with $T_c$ the number of traits contributing to competition, $t_{i, c}$ trait $c$ of species $i$ and $t_{j, c}$ trait $c$ of species $j$. If want to take the contribution of traits to competition we have to modify the trait values as follow:

\begin{gather}
\delta_{ij} = \sqrt{\sum_{c = 1}^{T_c} \left[ \sqrt{w_c} (t_{i, c} - t_{j, c})\right]^2}\\
\delta_{ij} = \sqrt{\sum_{c = 1}^{T_c} w_c \times (t_{i, c} - t_{j, c})^2},
\end{gather}
...
\begin{equation}
R_{i, x} = \sum_{g = 1}^{T_g} w_g \times k \times \exp\left(- \frac{(t_{i, g} - e_x)^2}{2\times {l_x}^2} \right)
\end{equation}

with $g$ the trait number, $0 \leq w_g \leq 1$ the contribution of this trait to growth (and $\sum_{g = 1}^{T_g} w_g = 1$), $t_{i, g}$ the trait number $g$ of species $i$.
...
R_{i, x, \text{hierarch.}} = H \sum_{h = 1}^{T_h} w_h \times f_H(t_{i, h}),
\end{gather}

with $w_g$ the weight of traits contributing to growth, $w_h$ the weight of traits contributing to hierarchical competition (as many as $T_h$, with $\sum_{h = 1}^{T_h} w_h = 1$). The function $f_H()$ is the hierarchical function that adjusts the importance of hierarchical traits. The idea is that species with low values of traits are disadvantaged while species with high trait values are advantaged. We want to consider $f_H()$ as being linear with a median value of 0 and be scaled between -1 and 1 for easier scaling by $H$. We can achieve this with the following function:
...
\begin{equation}
R_{i, x, \text{hierarch.}} = H \sum_{h = 1}^{T_h} w_h \times
...
\begin{equation}
R_{i, x} = k \sum_{g = 1}^{T_g} w_g \times
           \exp\left(- \frac{(t_{g,i} - e_x)^2}{2\times {l_x}^2} \right) +
           H \sum_{h = 1}^{T_h} w_h \times
           \frac{t_{i, h} - \min(t_h)}{\max(t_h) - \min(t_h)} \times 2 - 1,
\end{equation}
...
\begin{equation}
N_{t+1, i, x} = \frac{
    \left[ k \displaystyle\sum_{g = 1}^{T_g} w_g \times \exp\left(- \displaystyle\frac{(t_{i, g} - e_x)^2}{2\times {l_x}^2} \right) +
    H \sum_{h = 1}^{T_h} w_h \times
        \frac{t_{i, h} - \min(t_h)}{
        \max(t_h) - \min(t_h)} \times 2 - 1 \right] \times N_{t, i, x}}{
    1 + A \displaystyle\sum_{j = 1,~j \neq i}^{S} N_{t, j, x} [\max(\delta_{ij}) - \sqrt{\sum_{c = 1}^{T_c} w_c \times (t_{i, c} - t_{j, c})^2}] +
B \times N_{t,i,x}
} + D_{t, i, x}
\end{equation}
