...
f_H(t_{i, h}) = \frac{t_{i, h} - \min(t_h)}{\max(t_h) - \min(t_h)} \times
                [1 - (-1)] + (-1) \\
f_H(t_{i, h}) = \frac{t_{i, h} - \min(t_i)}{ \max(t_h) - \min(t_h)} \times
                2 - 1
\end{gather}

The fraction gets scaled is in $[0, 1]$, it gets multiplied by 2 so it is in $[0, 2]$ minus one which gets to $[-1, 1]$.
