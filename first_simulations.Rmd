...
traits <- generate_traits(species,
                          ...
                          rep(max(env), n_traits))
set.seed(1)
traits <- cbind(traits, trait2 = sample(traits[, 1]))

# Pairs of traits
trait_types = expand.grid(trait1 = c("R", "A", "RA", "none"),
                          trait2 = c("R", "A", "RA", "none"))

# trait_types = as.data.frame(t(combn(c("R", "A", "RA", "none"), 2)))
# colnames(trait_types) <- c("trait1", "trait2")

two_traits <- apply(trait_types, 1,
                    function(x){
                        two_traits_types = multigen(traits = traits, trait_type = x,
                                                    env = env, time = time, species = species,
                                                    patches = patches, composition = composition,
                                                    A = A, d = d, k = k, width = width)
                    })
names(two_traits) <- paste(trait_types[, 1], trait_types[, 2], sep = "_")

plot_patches_list <- lapply(names(two_traits),
                            function(x){
                                plot_patch(two_traits[[x]]$compo, 15, 150) +
                                    labs(subtitle = x)
                            })


plot_grid(plotlist = plot_patches_list,
          ncol = 4, nrow = 4)

# Plot moments
lapply(names(two_traits),
       function(x){
           compute_moments(two_traits[[x]], traits)
       })
both_cw = compute_moments(single_trait_both, traits)
colnames(both_cw) = c("patch", "both_cwm", "env", "both_cwv")
