...
sp_ab_gr <- function(simul, n_patches, sp, time){
    ...
    env_hierarch_growth <- gather(env_hierarch_growth, "sp", "r_env_hierarch",
                                  contains("species"))
    env_hierarch_growth$sp <- gsub("species", "sp", env_hierarch_growth$sp)

    obs_ab$env <- 1:n_patches
    obs_ab <- gather(obs_ab, "sp", "obs_ab", contains("species"))
    obs_ab$sp <- gsub("species", "sp", obs_ab$sp)

    env_ab$env <- 1:n_patches
    env_ab <- gather(env_ab, "sp", "env_ab", contains("species"))
    ...
    max_r_env_hierarch <- env_hierarch_growth %>%
        group_by(sp) %>%
        top_n(1, r_env_hierarch) %>%
        rename(max_r_env_hierarch = env) %>%
        as.data.frame()

    max_obs_ab <- obs_ab %>%
        group_by(sp) %>%
        top_n(1, obs_ab) %>%
        rename(max_obs_ab = env) %>%
        as.data.frame()

    max_env_ab <- env_ab %>%
        group_by(sp) %>%
        top_n(1, env_ab) %>%
        rename(max_env_ab = env) %>%
        as.data.frame()
    ...
    env_hierarch_growth <- left_join(
        env_hierarch_growth,
        max_r_env_hierarch[, c("sp", "max_r_env_hierarch")],
        by = "sp")

    obs_ab <- left_join(obs_ab, max_obs_ab[, c("sp", "max_obs_ab")],
                        by = "sp")

    env_ab <- left_join(env_ab, max_env_ab[, c("sp", "max_env_ab")],
                        by = "sp")

    # Merging all the dataframes together
    all_dat <- left_join(env_growth, env_hierarch_growth, by = c("env", "sp"))
    all_dat <- left_join(all_dat, obs_ab, by = c("env", "sp"))
    all_dat <- left_join(all_dat, env_ab, by = c("env", "sp"))
