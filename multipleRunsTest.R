...
trait1RAlist <- vector("list", length=25)
trait1RandAlist <- vector("list", length=25)
trait2RAlist <- vector("list", length=25)
...
trait_type <- c("RA", "RA") #rep("RA", n_traits)## # #
...
trait1RandAlist[[ff]] <- rbind(final_FD$nbsp, final_FD$FRic, final_FD$FEve, final_FD$FDiv, final_FD$FDis, final_FD$RaoQ)
...
library(plyr)
apply(ldply(lapply(trait1RandAlist, FUN=function(X){X$FDiv})), 2, "mean", na.rm=TRUE)
apply(ldply(lapply(trait1RandAlist, FUN=function(X){X$FDis})), 2, "mean", na.rm=TRUE)
apply(ldply(lapply(trait1RandAlist, FUN=function(X){X$FEve})), 2, "mean", na.rm=TRUE)
