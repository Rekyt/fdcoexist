...
meta_simul_A_B_and_k = function(seed_number, given_k = k, given_A = A,
                                given_B_over_A = B/A) {
    set.seed(seed_number)
    uncor_traits = generate_cor_traits(n_patches, n_species, n_traits - 1,
                                       cor_coef = 0)
...
list_A = 10^-(seq(2, 6, length.out = 10))
list_k = seq(1, 1.5, length.out = 10)
list_B_over_A = 10^seq(0, 4, length.out = 10)
...
var_a_var_k_var_ba = cross(list(run_n = seq(1), A = list_A, k = list_k,
    ...
    meta_simul_A_B_and_k(x$run_n, x$k, x$A, x$B_over_A) %>%
        mutate(k = x$k, A = x$A, B_over = x$B_over_A)
}, .progress = TRUE)
tictoc::toc()
...
plot_k_A = var_ba_df %>%
    ...
    labs(x = "k (maximum growth rate)",
         ...
         subtitle = "Average Richness over all values of B/A") +
...
plot_A_BA = var_ba_df %>%
    ...
    labs(x = "A (interspecific competition)",
         ...
         fill = "Species Richness\n(at t = 150)",
         subtitle = "Average Richness over all values of k") +
    theme(legend.position = "top")
...
plot_k_BA = var_ba_df %>%
    ...
    labs(x = "k (maximum growth rate)",
         y = "B/A (ratio intra/inter)",
         fill = "Species Richness\n(at t = 150)",
         subtitle = "Average Richness over all values of A") +
    theme(legend.position = "top")

plot_grid(plot_k_A, plot_A_BA, plot_k_BA, nrow = 3)
```

```{r biplot_with_R2_and_slope_values}
var_ba_r2 = var_ba_df %>%
    ungroup() %>%
    filter(!is.nan(trait2_cwm)) %>%
    nest(-k, -A,-B_over, -seed_number, -scenario) %>%
    mutate(data = data %>%
               map(~.x %>%
                       gather("cwm_name", "cwm_value", ends_with("cwm")) %>%
                       group_by(cwm_name, add = TRUE) %>%
...
plot_k_A_r2 = var_ba_r2 %>%
    ...
    labs(x = "k (maximum growth rate)",
         y = "A (interspecific competition)",
         fill = "CWM <-> Env. R^2",
         subtitle = "Average R-squared over all values of B/A") +
    theme(legend.position = "top")

plot_A_BA_r2 = var_ba_r2 %>%
    ggplot(aes(A, B_over)) +
    stat_summary_2d(aes(z = adj.r.squared), fun = "mean", geom = "raster") +
    scale_fill_viridis_c() +
    scale_x_log10() +
    ...
    labs(x = "A (interspecific competition)",
         y = "B/A (ratio intra/inter)",
         fill = "CWM <-> Env. R^2",
         subtitle = "Average R-squared over all values of k") +
    theme(legend.position = "top")

plot_k_BA_r2 = var_ba_r2 %>%
    ggplot(aes(k, B_over)) +
    stat_summary_2d(aes(z = adj.r.squared), fun = "mean", geom = "raster") +
    scale_fill_viridis_c() +
    scale_y_log10() +
    labs(x = "k (maximum growth rate)",
         y = "B/A (ratio intra/inter)",
         fill = "CWM <-> Env. R^2",
         subtitle = "Average R-squared over all values of A") +
    theme(legend.position = "top")



# Plotting CWM <-> Env. Slope
plot_k_A_slope = var_ba_slope %>%
    ggplot(aes(k, A)) +
    ...
    labs(x = "k (maximum growth rate)",
         y = "A (interspecific competition)",
         fill = "CWM <-> Env. Slope",
         subtitle = "Average Slope over all values of B/A") +
    theme(legend.position = "top")

plot_A_BA_slope = var_ba_slope %>%
    ggplot(aes(A, B_over)) +
    stat_summary_2d(aes(z = estimate), fun = "mean", geom = "raster") +
    scale_fill_viridis_c() +
    scale_x_log10() +
    ...
    labs(x = "A (interspecific competition)",
         y = "B/A (ratio intra/inter)",
         fill = "CWM <-> Env. Slope",
         subtitle = "Average Slope over all values of k") +
    theme(legend.position = "top")

plot_k_BA_slope = var_ba_slope %>%
    ggplot(aes(k, B_over)) +
    stat_summary_2d(aes(z = estimate), fun = "mean", geom = "raster") +
    scale_fill_viridis_c() +
    scale_y_log10() +
    labs(x = "k (maximum growth rate)",
         y = "B/A (ratio intra/inter)",
         fill = "CWM <-> Env. Slope",
         subtitle = "Average Slope over all values of A") +
    theme(legend.position = "top")
