...
low_k_simuls %>%
    bind_rows() %>%
    ggplot(aes(patch_optim, trait2_cwm, color = cor_level,
               group = interaction(cor_level, seed_number))) +
    geom_line(aes(group = interaction(cor_level, seed_number)), alpha = 1/25) +
    geom_smooth(aes(group = cor_level)) +
    facet_grid(rows = vars(compet_status, env_width), cols = vars(scenario)) +
    scale_color_viridis_d("Trait\nCorrelations", labels = lab_cor) +
    labs(x = "Environment",
         y = "Trait 2 CWM",
         subtitle = "CWM <-> Env. Relationship across all simulations")
...
plot_low_k_slope = ggplot(low_k_simuls_slope, aes(scenario, estimate)) +
    ...
    labs(subtitle = paste0("Slope of trait2 CWM ~ environment; ",
                           low_k_run_char),
         x = "Scenario",
         y = "Slope") +
