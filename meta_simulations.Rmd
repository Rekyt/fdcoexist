...
low_k_simuls = future_map(seq(total_runs), function(x) {
    ...
    meta_simul2(x, given_k = low_k)
}, .progress = TRUE)
tictoc::toc()

low_k_simuls_r2 = low_k_simuls %>%
    map(~.x %>%
...
plot_low_k_r2 = ggplot(low_k_simuls_r2, aes(scenario, r.squared)) +
    ...
    labs(subtitle = paste0("R2 of trait2 CWM ~ environment; ", low_k_run_char),
         x = "Scenario",
         y = expression("R"^2)) +
