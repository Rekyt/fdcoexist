...
low_k = 1.15
low_k_run_char = paste0("N = ", total_runs, "; A = ", A, "; B = ", B, "; k = ",
                        low_k)

# Actual simulation -----------------------------------------------------------
plan(multiprocess)
tictoc::tic()
low_k_simuls = future_map(seq(total_runs), function(x) {
    devtools::load_all()
...
low_k_simuls_slope = low_k_simuls %>%
    map(~.x %>%
...
```{r low_k_paper_figures}
# Simpler Figures ----------------------------------------------------------------------
# Show shape of CWM <-> across situations
plot_low_k_cwm_env = low_k_simuls %>%
    bind_rows() %>%
...
plot_low_k_slope_constant_env = low_k_simuls_slope %>%
...
plot_low_k_var_constant_env = low_k_simuls_var %>%
    filter(env_width != "varying", compet_status == "compet",
           !(scenario %in% c("R75A25", "R25A75"))) %>%
...
plot_low_k_slope_constant_env
plot_low_k_var_constant_env

plot_low_k_slope_var_env = low_k_simuls_slope %>%
    filter(env_width == "varying", compet_status == "compet",
           !(scenario %in% c("R75A25", "R25A75"))) %>%
    ggplot(aes(scenario, estimate)) +
    geom_boxplot(aes(fill = fct_rev(cor_level)),
...
plot_low_k_var_var_env = low_k_simuls_var %>%
    filter(env_width == "varying", compet_status == "compet",
           !(scenario %in% c("R75A25", "R25A75"))) %>%
    ggplot(aes(scenario, var_value)) +
    geom_boxplot(aes(fill = fct_rev(cor_level)),
...
plot_grid(plot_low_k_slope_var_env,
          plot_low_k_var_var_env, nrow = 2, align = "hv")
```

```{r low_k_complex_figures, eval=FALSE}
plot_low_k_slope = ggplot(low_k_simuls_slope, aes(scenario, estimate)) +
    geom_boxplot(aes(fill = fct_rev(cor_level)),
                 position = position_dodge(width = 0.9)) +
...
plot_low_k_var = ggplot(low_k_simuls_var, aes(scenario, var_value)) +
    geom_boxplot(aes(fill = fct_rev(cor_level)),
                 position = position_dodge(width = 0.9)) +
    ...
    ylim(0, NA) +
    scale_fill_viridis_d("Trait\nCorrelations", labels = lab_cor) +
    labs(subtitle = paste0("Trait Variance across Patches; ", low_k_run_char),
         x = "Scenario",
         y = "Trait Variance among patches") +
    theme(axis.text.x = element_text(angle = 45, hjust = 1),
          legend.position = "top")
