...
# Simulations ------------------------------------------------------------------
three_traits <- multigen(traits = traits,
...
# Plot individual patches 
plot_single_patch <- directlabels::direct.label(plot_patch(three_traits$compo,
                                                           1, 150))
...
```{r}
all_land = landscape_df(three_traits, trait_types) %>%
    inner_join(traits %>%
                   as.data.frame() %>%
                   rownames_to_column("species"),
               by = "species") %>%
