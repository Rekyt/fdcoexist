...
list_df <- mapply(cbind, list_df, "var_df" = names(list_df),
                  SIMPLIFY = FALSE)
list_df <- lapply(list_df, function(x) data.frame(x, type = "constant"))
...
list_var_df <- mapply(cbind, list_var_df, "var_df" = names(list_var_df),
                      SIMPLIFY = FALSE)
list_var_df <- lapply(list_var_df, function(x) data.frame(x, type = "varying"))

all_list <- c(list_df, list_var_df)

all_df <- as.data.frame(bind_rows(all_list))

# List of weighted linear regressions
trait2_w_lm = all_df %>%
    group_by(var_df, scenario) %>%
    do(model = lm(trait2 ~ patch_optim, data = ., weights = N))

list_lm_df <- trait2_w_lm[[1]]
list_lm_scenario <- trait2_w_lm[[2]]

trait2_w_lm <- lapply(trait2_w_lm$model, summary)

synth_plot <- data.frame(df = list_lm_df,
                         scenario = list_lm_scenario,
                         r2 = unlist(lapply(trait2_w_lm,
                                            function(x) x$r.squared)))

synth_plot$df <- as.factor(synth_plot$df)
synth_plot$df <- factor(synth_plot$df,
                        levels(synth_plot$df)[c(6:1)])
synth_plot$compet <- ifelse(grepl("no_compet", synth_plot$df), "No", "Competition")
synth_plot$type <- ifelse(grepl("_var_", synth_plot$df), "Varying", "Constant")

print(ggplot(synth_plot, aes(as.factor(df), r2)) +
          geom_point(aes(shape = as.factor(scenario),
                         fill = as.factor(compet)),
                     color = "black", size = 4) +
          scale_fill_manual("Competition", values = c(No = "#a50026",
                                                      Competition = "#313695")) +
          scale_shape_manual("Scenario", values = c(RA100100 = 21, RA2575 = 22,
                                                    RA5050 = 23, RA7525 = 24)) +
          facet_wrap(~ type) +
          labs(subtitle = "R2 for the weighted linear regression between trait2 and environment",
               x = "Scenario",
               y = expression("R"^2)) +
