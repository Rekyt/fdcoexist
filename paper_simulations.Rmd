...
mod_df <- all_df %>%
    ...
    summarise(patch_optim = unique(patch_optim), trait2_cwm = sum(trait2_cwm)) %>%
    nest() %>%
    mutate(mod_cwm_env = map(data, ~lm(trait2_cwm ~ patch_optim, data = .x)),
...
plot_r2_cwm_env = ggplot(mod_df, aes(scenario, r.squared)) +
    geom_point(aes(fill = fct_rev(cor_level)),
               ...
               shape = 21) +
    facet_grid(rows = vars(compet_status), cols = vars(env_width),
               labeller =
