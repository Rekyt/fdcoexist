---
title: "Full Simulations for the paper"
author:
    - Pierre Denelle
    - Matthias Grenié
    - Cyrille Violle
...
facet_scenar = facet_wrap(vars(scenario), labeller = as_labeller(trait_scenar_names))
```
...
plot_no_compet_no_cor_abund_env = ggplot(no_compet_no_cor_df %>%
    ...
    labs(x = "Environmental value",
         ...
         subtitle = no_compet_no_cor_label)

plot_no_compet_no_cor_abund_env
```

```{r no_compet_no_cor_cwm_env}
plot_no_compet_no_cor_cwm_env = no_compet_no_cor_df %>%
    filter(N >= 2) %>%
...
plot_no_compet_no_cor_cwm_env_abund = no_compet_no_cor_df %>%
    group_by(scenario, patch) %>%
...
plot_no_compet_low_cor_abund_env = ggplot(no_compet_low_cor_df %>%
           filter(N >= 2), aes(patch_optim, N)) +
    geom_point(shape = ".") +
    geom_smooth() +
    facet_scenar +
    labs(x = "Environmental value",
         y = "Species Abundance (gen. = 150)",
         subtitle = no_compet_low_cor_label)

plot_no_compet_low_cor_abund_env
```

```{r no_compet_low_cor_cwm_env}
plot_no_compet_low_cor_cwm_env = no_compet_low_cor_df %>%
    filter(N >= 2) %>%
    group_by(scenario, patch) %>%
    summarise_at(vars(contains("cwm")), sum, na.rm = TRUE) %>%
    mutate(patch_optim = gsub("patches", "", patch) %>%
...
plot_no_compet_low_cor_cwm_env_abund = no_compet_low_cor_df %>%
    group_by(scenario, patch) %>%
    summarise_at(vars(contains("cwm"), N), sum) %>%
    mutate(patch_optim = gsub("patches", "", patch) %>%
               as.numeric()) %>%
...
plot_no_compet_low_cor_df_ind_landscape = no_compet_low_cor_df %>%
    select(-contains("cwm")) %>%
    gather("trait_name", "trait_value", contains("trait")) %>%
    ggplot(aes(patch_optim, trait_value, z = N)) +
    stat_summary_2d(bins = 17) +
    facet_grid(vars(scenario), vars(trait_name),
               labeller = labeller(scenario = trait_scenar_names,
                                   trait_name = function(x) gsub("_", " ", x))) +
    scale_fill_viridis_c() +
...
no_compet_high_cor_label = paste0("No competition; ",
                                d * 100, "% dispersal;",
                                " 3 low correlated (r = 0.7) traits")
```

```{r no_compet_high_cor_abund_env}
plot_no_compet_high_cor_abund_env = ggplot(no_compet_high_cor_df %>%
           filter(N >= 2), aes(patch_optim, N)) +
    geom_point(shape = ".") +
    geom_smooth() +
    facet_scenar +
    labs(x = "Environmental value",
         y = "Species Abundance (gen. = 150)",
         subtitle = no_compet_high_cor_label)

plot_no_compet_high_cor_abund_env
```

```{r no_compet_high_cor_cwm_env}
plot_no_compet_high_cor_cwm_env = no_compet_high_cor_df %>%
    filter(N >= 2) %>%
    group_by(scenario, patch) %>%
    summarise_at(vars(contains("cwm")), sum, na.rm = TRUE) %>%
    mutate(patch_optim = gsub("patches", "", patch) %>%
...
plot_no_compet_high_cor_cwm_env_abund = no_compet_high_cor_df %>%
    group_by(scenario, patch) %>%
    summarise_at(vars(contains("cwm"), N), sum) %>%
    mutate(patch_optim = gsub("patches", "", patch) %>%
               as.numeric()) %>%
...
plot_no_compet_high_cor_df_ind_landscape = no_compet_high_cor_df %>%
    select(-contains("cwm")) %>%
    gather("trait_name", "trait_value", contains("trait")) %>%
    ggplot(aes(patch_optim, trait_value, z = N)) +
    stat_summary_2d(bins = 17) +
