...
list_df <- mapply(cbind, list_df, "df" = names(list_df), SIMPLIFY = FALSE)

all_df <- as.data.frame(bind_rows(list_df))
...
print(ggplot(synth_plot, aes(as.factor(df), r2)) +
          geom_point(aes(shape = as.factor(scenario), fill = as.factor(scenario)),
                     color = "black", size = 4) +
          scale_fill_manual("Scenario", values = c(RA100100 = viridis::viridis(4)[1],
                                                   RA2575 = viridis::viridis(4)[2],
                                                   RA5050 = viridis::viridis(4)[3],
                                                   RA7525 = viridis::viridis(4)[4])) +
