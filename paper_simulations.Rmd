...
trait_scenarios = list(
    RA00     = data.frame(trait = paste0("trait", 1:3),
                          growth_weight = c(1, 0, 0),
                          compet_weight = c(0, 0, 1)),
    RA1090   = data.frame(trait = paste0("trait", 1:3),
    ...
    RA9010   = data.frame(trait = paste0("trait", 1:3),
...
$$
N_{t+1, i, x} = \frac{
k \times \exp\left(- \displaystyle\frac{(trait_i - env_x)^2}{2\times {width}^2}\right) \times N_{t, i, x}}{
1 + A \times \displaystyle\sum_{j = 1, j \neq i}^{S} N_{t, j, x} \times (1 - \delta_{ij})
}
$$

NB: When $i$ = $j$, the species faces intra-specific competition leading to:
$$\delta_{ii} = \frac{K}{(K - N_{t,i,x})}$$
...
N_{t+1, i, x} = \frac{
    ...
    1 + A \times \displaystyle\sum_{j = 1,~j \neq i}^{S} N_{t, j, x}  (1 - \delta_{ij}) +
        B \times \displaystyle\sum_{j = 1,~j = i}^{S}\frac{K}{K - N_{t, j, x}}
}
$$

with $A$ the coefficient scaling inter-specific competition and $B$ the one for intra-specific competition. Depending on the scenario we can suppose that $B = A$
